...
class Scanner {
  ...
  scanAuto(outputPath) {
    ...
    try {
      // 스캔마다 내부 상태/버퍼가 남는 경우가 있어 초기화 시도
      try {
      ...
      if (result === 0x00) {
        console.log('[Scanner] Scan success (passport mode)');
        return { success: true, type: 'passport' };
      }
      
      // 2. 여권 실패 시 신분증 스캔 시도 (ScanCard)
      if (result === 0xf8 || result === 0xc7) {
        ...
        // 카드 스캔 모드(nType)가 문서 종류(주민등록증/면허증) 등에 따라 다를 수 있어 여러 값 시도
        const cardTypesToTry = [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07];
        for (const cardType of cardTypesToTry) {
          result = functions.ScanCard(outputPath, cardType);
          if (result === 0x00) {
            console.log('[Scanner] ScanCard success (cardType=0x' + cardType.toString(16) + ')');
